...
(async () => {
	...
	const esbuildConfig = 'esbuild' in pkgConfig ? pkgConfig.esbuild : pkgConfigDefaultEntries.esbuild;
	const binPath = dirname(pkgConfig.bin);
	const status_minifying = oraStatus(`Minifying '${pkgConfig.main}' into '${pkgConfig.bin}'...`);
	await esbuildMinify(pkgConfig.main, binPath, { ...pkgConfigDefaultEntries.esbuild, ...esbuildConfig });
